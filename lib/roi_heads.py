# modified from https://github.com/pytorch/vision/blob/release/0.11/torchvision/models/detection/roi_heads.py
import torch
import torchvision
...
def maskrcnn_inference(x, labels):
    ...
    """
    #print("input mask prob: ", mask_prob.shape)
    ...
    #print("mask prob: ", mask_prob[0].shape)
    """
    return mask_prob
...
def project_masks_on_boxes(gt_masks, boxes, matched_idxs, M):
    ...
    matched_idxs = torch.zeros(boxes.shape[0]).to(boxes)
    rois = torch.cat([matched_idxs[:, None], boxes], dim=1)
    #gt_masks = gt_masks[:, None].to(rois)
    gt_masks = gt_masks[None, :].to(rois)
    #return roi_align(gt_masks, rois, (M, M), 1.)[:, 0]
    return roi_align(gt_masks, rois, (M, M), 1.)
...
def paste_mask_in_image(mask, box, im_h, im_w):
    ...
    im_mask = torch.zeros((mask.shape[0], mask.shape[1], im_h, im_w), dtype=mask.dtype, device=mask.device)
    x_0 = max(box[0], 0)
    x_1 = min(box[2] + 1, im_w)
    y_0 = max(box[1], 0)
    y_1 = min(box[3] + 1, im_h)
    
    for i, b_m in enumerate(mask):
        for j, m in enumerate(b_m):
            im_mask[i, j, y_0:y_1, x_0:x_1] = mask[i, j,
